...
describe("<HeaderLogged />", () => {
  ...
  it("should render 3 dots when there not user logged", async () => {
    router.push("/");
    await router.isReady();
    const wrapper = shallowMount(HeaderLogged, {
      global: {
  ...
  it("should render user anem when there user logged", async () => {
    router.push("/");
    await router.isReady();
    mockStore.currentUser.name = "Jean";
    const wrapper = shallowMount(HeaderLogged, {
      global: {
        plugins: [router],
      },
    ...
    const buttonLogout = wrapper.find("#logout-button");
    expect(buttonLogout.text()).toBe("Jean (sair)");
  });
});
